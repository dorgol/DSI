...
A baseline measure of the centrality of a journal could be its total degree, the sum of its in- and out- degrees. A baseline measure of the importance of a journal could be its in-degree, which in this case means the number of citations it receives from other journals. Below we compare several centrality metrics in `igraph` to these baselines.
...
- **PageRank** - "The Google algorithm". Addresses limitation of Eigenvector centrality for directed networks by giving every node a minimum weight. Also differentiates between ``important'' nodes that point to few vs. many other nodes. Roughly, the number of times a theoretical web surfer visits a given page. Popular for web search and beyond because it is is straightforward, fast and guaranteed to come to a unique solution (@gleich14). (Eigenvector centrality may not.)
...
\end{aligned}
$$
where we let $k^{out}_j$ be 1 where it's 0, and $D$ is a matrix whose entries are zero except the diagonal, where $D_{ii} = max(1, k^{out}_i)$, i.e. the out-degree of each node or 1 if the node's degree is 0. There are versions where $\beta$ is customized for each node, i.e. $\beta_i$. Guaranteed solution.
...
In general, centrality functions on weighted graphs will use the graph's `weights` variable by default.
...
In the latter the number and size of groups is set beforehand, and goal is to minimize edges between groups. Goal is similar, more general in the former, "find the natural fault lines" (@newman10).
...
where $g(i)$ is the group of node $i$ after the cut. If an undirected graph, a factor of $1/2$ is added to the expression, though the minimal solution is unaffected.

In `igraph` the `min_cut` is an implementation of this. 
...
We may want to set the group size. A simple to implement (though difficult to explain) method is based on an eigenvector of the Laplacian of the graph, a matrix formed using the adjacency matrix and diagonal matrix of nodal degrees (usually $D - A$). It turns out that this matrix describes graph structure, and its second-smallest eigenvector is valuable in finding a partition with a small $R$ (cut size). 

```{r journals_spectral, eval = T, cache = T, fig.width = 10}

# Spectral method
N = 47
eigen_laplace = eigen(laplacian_matrix(Cgraph))

# Spectral method over all sizes
mod.spec = 1:floor(N/2)
mod.random = 1:floor(N/2)

for(i in 1:floor(N/2)) {
  size.smaller = i
  mem.spec = as.numeric(rank(eigen_laplace$vectors[,N-1]) > size.smaller) + 1
  mem.random = sample(c(rep(1,i), rep(2, N-i)), N, replace = F)
  mod.spec[i] = modularity(Cgraph.sym, membership = mem.spec) #weights incorporated automatically, modularity for undirected only
  mod.random[i] = modularity(Cgraph.sym, membership = mem.random)
}

{plot(1:floor(N/2), mod.spec, type = "l", ylim = c(-.025,.025), ylab = "modularity", xlab = "size of smaller group")
points(1:floor(N/2), mod.random, col = "red", type ="l")
legend("topleft", legend = c("spectral partition of laplacian", "random partition"), col=c(1,2), lty=1)}

size.smaller = 15
gp.spectral.membership = as.numeric(rank(eigen_laplace$vectors[,N-1]) > size.smaller) + 1
...
Modularity is another way to formalize what it means to be a "community". Like cut size it looks to break relatively few edges when assigning communities, but it evaluates "few" relative to expectation. It tells us how tight communities are by measuring, roughly, how many more edges occur within communities than we would expect randomly, given the nodal degrees.

$$\text{modularity (Q)} = \frac{1}{2m}\sum_{ij}(A_{ij}-\frac{k_ik_j}{2m})\delta(c_i,c_j),  $$
where $m$ is the number of edges in the network, $\delta(c_i,c_j)$ is 1 if $i$ and $j$ are in the same community and 0 otherwise. \frac{k_ik_j}{2m} represents the expected number (or value) of edges between vertices $i$ and $j$, based on a hypergeometric distribution. The maximum value occurs when nodes are only connected if in the same group

Finding the graph split guaranteed to have the highest modularity is very slow for large graphs. 
...
``Local'' calculation may be faster and more suitable for very large networks. 
...
The following plots compare output of the different clustering algorithms by coloring the nodes accordingly. For consistency all clustering algorithms are applied to the symmetric version of the journal graph where the edge weights are the total citations between two journals.
...
# In general they defaults to use weight so weights argument not necessary
...
`igraph` has a cluster_optimal function to do an exhaustive search for the highest-modularity communities, but this will ONLY work if you install the development version if igraph, see: https://github.com/igraph/rigraph

```{r journals_cluster_optimal, cache = T, eval = F, echo = T}
...
- Reciprocity: 
  ...
  + $\sum_{i, j} (B_{ij}B'_{ij}) / sum_{ij}(B_{ij})$ , where $B_ij$ is the binary representation of $A_{ij}$. 
  + To account for valued networks can use a simple correlation measure.
...
# For weighted graphs, can use a simple correlation measure
cor(as.vector(Cmatrix), as.vector(t(Cmatrix)))
```

- Assortativity:
    + **by degree**: Do higher-degree nodes connect to other higher degree nodes? 
...
```{r journals_clustering_coef, eval = T, cache = T}
transitivity(Cgraph, type = "global") #directed graphs treated as undirected
...
- Using the journal data, find apply a measure for the following not discussed
