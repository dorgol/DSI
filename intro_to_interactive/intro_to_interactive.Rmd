...
```{r texas_housing_plotly, eval = T, echo = T, warning = F, message = F, cache =  T, results = "hide"}
...
pizza.datatable = datatable(shared_pizza_delivered_small, options = list(pageLength = 5),
                            selection = "multiple", height = "200px", rownames = F)

# 4. Panel plot ----
# unlike subplot, bscols naturally joins widgets of different types
pizza_linked = bscols(widths = c(8,4), plotly.map.emoji, list(plotly.pizzas.delivered, pizza.datatable))
...
txhousing.subset = txhousing %>%
  filter(year > 2006 & year < 2009,
         city %in% c("Dallas", "Houston", "San Antonio")) %>%
